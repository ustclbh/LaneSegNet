...
class Alaug(object):
    ...
    def __init__(self, transforms):
        ...
        self.__augmentor_img_only = None
        # put transforms in a list
        self.transforms = []
        self.transforms_img_only = []
    ...
    def build(self):
        if len(self.transforms) == 0:
            return
        # Geometric transforms with additional targets
        self.__augmentor = al.Compose(
            self.transforms,
            bbox_params=self.bbox_params,
            keypoint_params=self.keypoint_params,
            additional_targets={'lane_seg': 'image'}
        )
    ...
    def __call__(self, data):
        ...
        if 'lane_seg_label' in data:
            lane_seg_label = data['lane_seg_label']
        ...
        # Apply geometric transforms to both image and label
        aug = self.__augmentor(
            ...
            lane_seg=lane_seg_label if lane_seg_label is not None else img)

        data['img'] = aug['image']
        if lane_seg_label is not None:
            data['lane_seg_label'] = aug['lane_seg']
