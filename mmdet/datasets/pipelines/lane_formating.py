...
class CollectLanePoints(Collect):
    ...
    def target(self, results):
        ...
        def lane_seg_map_to_offset_map(lane_seg_map, num_sample_rows, downsample_factor=8.0):
            '''
            #input:
            lane_seg_map  1xHxW with label 1 ,2 ,3 4
            sample_rows  N  num of rows to sample evenly spaced lane pts from 0 to H

            #output:
            offset_map NxHxW horizontal offset of each lane pts to the corresponding lane pts on the sampled rows

            description:
            给定某行的索引，计算该行上的所有车道点，计算其他行上的车道点相对于各自车道点的水平偏移，形成一个相对偏移图
            '''
            if isinstance(lane_seg_map, DC):
                lane_seg_map = lane_seg_map.data[0]  # 获取实际数据 
            H, W = lane_seg_map.shape[-2:]
            sample_rows = np.linspace(0, H-1, num_sample_rows).astype(np.int32)
            
            offset_maps = []
            # For each lane label
            for lane_id in range(1, 5):
                lane_mask = (lane_seg_map == lane_id)
                offset_map = np.zeros((num_sample_rows, int(H//downsample_factor), W  ))
                # Get points for each row
                for i, ref_row in enumerate(sample_rows):
                    # Find reference point on sample row
                    ref_points = np.where(lane_mask[ ref_row, :])[0]
                    if len(ref_points) == 0:
                        continue
                    ref_point = ref_points[len(ref_points)//2]  # Take middle point if multiple
                    
                    # Calculate offset for all points in this lane
                    for row in sample_rows:
                        points = np.where(lane_mask[row, :])[0]
                        if len(points) == 0:
                            continue
                        # point = points[len(points)//2]  # Take middle point if multiple
                        offset_map[i, row//int(downsample_factor), lane_mask[row, :]] = (points - ref_point)/downsample_factor #40x40x800
                offset_maps.append(offset_map)
            offset_map = np.sum(offset_maps, axis=0)

            indices = np.linspace(0, offset_map.shape[2]-1, 100).astype(int)
            offset_map = offset_map[:, :, indices]
            ...
            #     plt.imshow(offset_map[i], cmap='viridis')
            #     plt.title(f'Row {sample_rows[i]}')
            # plt.tight_layout()
            # plt.savefig('new_offset_map.png')
            return offset_map
        ...
        # Process lane segmentation label if available
        if 'lane_seg_label' in results:
            rela_offset_maps = lane_seg_map_to_offset_map(results['lane_seg_label'], hm_h)
            if rela_offset_maps is not None:
                results['rela_offset_maps'] = DC(to_tensor(rela_offset_maps).float(), stack=True, pad_dims=None)
